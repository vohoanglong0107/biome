...
use biome_graphql_syntax::{
    AnyGraphqlTypeDefinition, AnyGraphqlTypeExtension, GraphqlFragmentDefinition, GraphqlLanguage,
...
use biome_rowan::{syntax::Preorder, AstNode, TokenText};
...
impl SemanticEventExtractor {
    ...
    pub fn leave(&mut self, node: &GraphqlSyntaxNode) {
        match node.kind() {
            GRAPHQL_ROOT => {
                self.resolve_binding_references();
    ...
    fn resolve_binding_references(&mut self) {
    ...
    fn resolve_scope_implicit_variables_references(
        ...
        println!("Resolving scope {}", current_scope_id);
    ...
    fn next(&mut self) -> Option<Self::Item> {
        loop {
            if let Some(e) = self.extractor.pop() {
                ...
                use biome_graphql_syntax::WalkEvent::*;
...
/// use biome_graphql_syntax::*;
/// use biome_graphql_semantic::*;
/// let tree = parse_graphql("let a = 1");
/// for e in semantic_events(tree.syntax()) {
///     dbg!(e);
/// }
/// ```
pub fn semantic_events(root: GraphqlSyntaxNode) -> impl IntoIterator<Item = SemanticEvent> {
